...
function getToolLabel(toolName: string, args: Record<string, unknown>, done: boolean): string {
  ...
  if (!command) return done ? "Done" : "Working on it...";

  if (toolName === "str_replace_editor") {
    const ops: Record<string, { pending: string; done: string }> = {
      view:        { pending: `Reading ${file}...`,   done: `Read ${file}` },
      create:      { pending: `Creating ${file}...`,  done: `Created ${file}` },
      str_replace: { pending: `Updating ${file}...`,  done: `Updated ${file}` },
      insert:      { pending: `Updating ${file}...`,  done: `Updated ${file}` },
      undo_edit:   { pending: `Reverting ${file}...`, done: `Reverted ${file}` },
    };
    const entry = ops[command];
    if (entry) return done ? entry.done : entry.pending;
  }

  if (toolName === "file_manager") {
    const ops: Record<string, { pending: string; done: string }> = {
      rename: { pending: `Renaming ${file}...`, done: `Renamed ${file}` },
      delete: { pending: `Removing ${file}...`, done: `Removed ${file}` },
    };
    const entry = ops[command];
    if (entry) return done ? entry.done : entry.pending;
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const badgeLabel = getToolLabel(tool.toolName, (tool as any).args ?? {}, !!isDone);
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {isDone ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{badgeLabel}</span>
                                  </>
                                ) : (
                                  <>
